...
const Storage = {
    ...
    seedDefaultData() {
        // Save empty collections directly to local storage to set clean seed baseline
        localStorage.setItem('inv_suppliers', JSON.stringify([]));
        localStorage.setItem('inv_products', JSON.stringify([]));
        localStorage.setItem('inv_purchases', JSON.stringify([]));
        localStorage.setItem('inv_stockouts', JSON.stringify([]));
        localStorage.setItem('inv_payments', JSON.stringify([]));
        ...
        const defaultLogs = [
            { 
                id: 'LOG_1', 
                type: 'system_init', 
                message: 'Crust & Chilly Inventory System initialized.', 
                userId: 'system', 
                userName: 'System', 
                date: this.getIndianDate(), 
                time: this.getIndianTime(), 
